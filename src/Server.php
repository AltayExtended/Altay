...
class Server{
	...
	private Config $moballowedworlds;

	private Config $mobnotallowedworlds;
	...
	public function getAltayVersion() : string{
	...
	public function allowedworlds(string $name) : bool{
		return $this->moballowedworlds->exists($name, true);
	}

	public function notallowedworlds(string $name) : bool{
		return $this->mobnotallowedworlds->exists($name, true);
	}
	...
	public function __construct(
		...
		try{
			...
			$altayYmlPath = Path::join($this->dataPath, "altay.yml");
			if(!file_exists($altayYmlPath)){
				$content = Utils::assumeNotFalse(file_get_contents(Path::join(\pocketmine\RESOURCE_PATH, "altay.yml")), "Missing required resource file");
				@file_put_contents($altayYmlPath, $content);
			}
			...
			@mkdir(Path::join($this->dataPath . "AutoMobSpawn/"));
			...
			$this->altayconfig = new Config(Path::join($this->dataPath, "altay.yml"), Config::YAML);
			$this->moballowedworlds = new Config(Path::join($this->dataPath, "AutoMobSpawn/allowedworlds.txt"), Config::ENUM);
			$this->mobnotallowedworlds = new Config(Path::join($this->dataPath, "AutoMobSpawn/notallowedworlds.txt"), Config::ENUM);

			$altayYmlPath = Path::join($this->dataPath, "altay.yml");
			$versioncontrol = $this->getAltayStringConfig("config-version");
			$altayversion = VersionInfo::ALTAY_VERSION;
			if($versioncontrol === $altayversion){
			}else{
				$content = Utils::assumeNotFalse(file_get_contents(Path::join(\pocketmine\RESOURCE_PATH, "altay.yml")), "Missing required resource file");
				@file_put_contents($altayYmlPath, $content);
			}
	...
	public function getAltayIntConfig(string $name) : int{
		return $this->altayconfig->get($name);
	}
	public function getAltayStringConfig(string $name) : string{
		return $this->altayconfig->get($name);
	}
	public function getAltayBoolConfig(string $name) : bool{
		return $this->altayconfig->get($name);
	}
